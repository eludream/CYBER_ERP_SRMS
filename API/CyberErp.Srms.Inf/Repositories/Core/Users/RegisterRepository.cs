...
using UserEntity = CyberErp.Srms.Dom.Entities.Core.User;
...
namespace CyberErp.Srms.Inf.Repositories.Core.Users
{
        ...
        public Task<RegisterResult> RegisterAsync(RegisterUserDto dto) =>
            RepositoryExecutor.ExecuteAsync(
                ...
                .FirstOrDefaultAsync();
                    ...
                    // Seed tenant data (modules, operations, roles, permissions, etc.)
                    await _tenantSeedService.SeedTenantDataAsync(tenant.Id.ToString(), user.Id.ToString());
                    _logger.LogInformation("Tenant data seeded for TenantId: {TenantId}", tenant.Id);
