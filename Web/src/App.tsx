...
const TenantProtectedRoute = ({ children, allowInactiveMembership = false }: { children: React.ReactNode; allowInactiveMembership?: boolean }) => {
  ...
  if (hasInactiveMembership && !currentTenant) return (
    <main className="flex min-h-screen items-center justify-center bg-background p-6"><section className="max-w-lg rounded-lg border bg-card p-6 text-center shadow-sm"><h1 className="text-xl font-semibold">Tenant access inactive</h1><p className="mt-2 text-sm text-muted-foreground">Your tenant membership or tenant is inactive. Contact your platform or organization administrator to reactivate your access.</p></section></main>
  );
  // Tenant membership, selection, role, and subscription gates are intentionally
  // disabled in development so core features can be tested independently.
  if (import.meta.env.DEV) return <Fragment key={currentTenant?.id ?? "no-tenant"}>{children}</Fragment>;
  ...
  if (!currentTenant) return (
    <main className="flex min-h-screen items-center justify-center bg-background p-6">
      <section className="max-w-lg rounded-lg border bg-card p-6 text-center shadow-sm">
        <h1 className="text-xl font-semibold">No tenant access</h1>
        <p className="mt-2 text-sm text-muted-foreground">Your account is authenticated but has no tenant membership. Ask a platform or organization administrator to grant access.</p>
      </section>
    </main>
  );
...
const SrmsPathGuard = () => {
  ...
  if (isInitializing) return <PageLoader />;
  if (!isAuthenticated) return <RootLoginRedirect />;
  if (!user?.isPlatformAdministrator) return <RootLoginRedirect />;
  return <Outlet />;
};
